...
export default function Search(): ReactElement<any> {
  ...
  return (
    <form className="w-full md:w-1/4 lg:w-1/3 relative inline-block" autoComplete="off" onSubmit={(e: FormEvent) => e.preventDefault()}>
      ...
      {searchResults.length > 0 && (
        <ul className="space-x-0 z-10 absolute bg-transparent grid grid-cols-1 mr-4 md:mr-0 top-full left-0 right-0">
          {searchResults.map(result => (
            <a href={result.item.link} key={result.item.link}>
              <li className="border-2 border-gray-300 p-2 block text-gray-600 bg-gray-50 w-full hover:bg-gray-200">{result.item.title}</li>
            </a>
          ))}
        </ul>
      )}
